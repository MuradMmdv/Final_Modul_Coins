...
const Bullion = () => {
 ...
 useEffect(()=>{
  const fetchAllBullian= async ()=>{
    try{
      const res= await axios.get("http://localhost:3002/Exclusive")
      setcoinB(res.data);
      console.log(res);
    }
    catch(err){
        ...
        <div className='coinsBul'>
            {coinB.map(coin=>(
               <div className='coinB' key={coin.id}>
                ...
                <img className='imgBul' src={`${process.env.PUBLIC_URL}./img/${coin.images}`} alt=''/>                <div className='descAndName'><h2 className='nameBul' onClick={() => handleOpen(coin.id)}>{coin.coinname}</h2>
